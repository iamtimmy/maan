...
namespace maan::aggregate
{
	...
	static bool is( lua_State* state, int index )
	{
		...
		const auto fn = [state, start_index]< typename... types >()
		{
			return check< 0, types... >( state, start_index );
		};
	...
	{
		utilities::visit_members( std::forward< decltype( value ) >( value ), [state]( auto&& ... members )
		{
